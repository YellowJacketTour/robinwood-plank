...
const PROFILE_MODULES=["welcome","status","music","video","game","custom","collection","about","friends","widgets","feed","comments"];
const cleanHandle=(value:string)=>value.toLowerCase().replace(/[^a-z0-9_-]/g,"").slice(0,24);
const text=(value:unknown,max:number)=>typeof value==="string"?value.trim().slice(0,max):"";
const youtubeUrl=(value:unknown)=>{const raw=text(value,500);if(!raw)return "";try{const u=new URL(raw),host=u.hostname.replace(/^www\./,"");let id="";if(host==="youtu.be")id=u.pathname.slice(1).split("/")[0];else if(host==="youtube.com"||host==="m.youtube.com"||host==="youtube-nocookie.com")id=u.searchParams.get("v")||(/^\/(embed|shorts)\//.test(u.pathname)?u.pathname.split("/")[2]:"");return /^[\w-]{6,20}$/.test(id)?`https://www.youtube.com/watch?v=${id}`:""}catch{return ""}};
const youtubeUrls=(value:unknown)=>text(value,4000).split(/[\s,]+/).map(youtubeUrl).filter(Boolean).filter((url,index,all)=>all.indexOf(url)===index).slice(0,8).join("\n");
...
export async function POST(request:Request){
 ...
 let previousLayout:string[]=PROFILE_MODULES,previousHidden:string[]=[];try{const saved=JSON.parse(existing?.layoutJson||"[]");previousLayout=Array.isArray(saved)?saved:Array.isArray(saved?.order)?saved.order:PROFILE_MODULES;previousHidden=Array.isArray(saved?.hidden)?saved.hidden:[]}catch{}
 const layout=Array.isArray(p.layout)?p.layout.filter((v):v is string=>typeof v==="string"&&PROFILE_MODULES.includes(v)).slice(0,PROFILE_MODULES.length):previousLayout;
 const hiddenModules=Array.isArray(p.hiddenModules)?p.hiddenModules.filter((v):v is string=>typeof v==="string"&&PROFILE_MODULES.includes(v)).slice(0,PROFILE_MODULES.length):previousHidden;
 ...
 const rawTheme:Record<string,unknown>=p.themeJson&&typeof p.themeJson==="object"?p.themeJson as Record<string,unknown>:{},hex=(v:unknown,fallback:string)=>/^#[0-9a-f]{6}$/i.test(String(v))?String(v):fallback,fonts=new Set(["Verdana","Georgia","Arial","Courier New"]),templates=new Set(["lounge","classic","midnight","neon"]),theme={template:templates.has(String(rawTheme.template))?String(rawTheme.template):"lounge",pageBackground:hex(rawTheme.pageBackground,"#24130b"),panelBackground:hex(rawTheme.panelBackground,"#f2dfbe"),textColor:hex(rawTheme.textColor,"#2b160d"),linkColor:hex(rawTheme.linkColor,"#6e2b0e"),headingColor:hex(rawTheme.headingColor,"#fff0cf"),accentColor:hex(rawTheme.accentColor,"#e4862a"),fontFamily:fonts.has(String(rawTheme.fontFamily))?String(rawTheme.fontFamily):"Verdana",showTop8:rawTheme.showTop8!==false};
 ...
 const customHtml=await cacheLayoutAssets(text(p.customHtml,20000),wallet),values={handle,displayName:text(p.displayName,40)||existing?.displayName||handle,bio:text(p.bio,500),hobbies:text(p.hobbies,500),interests:text(p.interests,500),music:text(p.music,500),heroes:text(p.heroes,500),lookingToMeet:text(p.lookingToMeet,500),avatarUrl:savedAvatar,mood:text(p.mood,40)||"feeling board",moodText:text(p.moodText,140)||"holding down the lumberyard.",customHtml,themeJson:JSON.stringify(theme),layoutJson:JSON.stringify({order:layout,hidden:hiddenModules}).slice(0,4000),featuredVideo:youtubeUrls(p.featuredVideo),moderationStatus:existing?.moderationStatus||(isAdmin||autoApprove?"approved":"pending")};
