...
const httpsUrl=(value:unknown)=>{const raw=text(value,500);if(!raw)return "";try{const u=new URL(raw);return u.protocol==="https:"?u.toString():""}catch{return ""}};
const avatarUrl=httpsUrl;
...
export const publicProfile=(p:typeof profiles.$inferSelect)=>({id:p.id,handle:p.handle,displayName:p.displayName,bio:p.bio,hobbies:p.hobbies,interests:p.interests,music:p.music,heroes:p.heroes,lookingToMeet:p.lookingToMeet,avatarUrl:p.avatarUrl?`/api/avatar?handle=${p.handle}`:"",bannerUrl:p.bannerUrl,mood:p.mood,moodText:p.moodText,customHtml:p.customHtml,mobileCss:p.mobileCss,themeJson:p.themeJson,layoutJson:p.layoutJson,featuredVideo:p.featuredVideo,viewCount:p.viewCount,createdAt:p.createdAt,updatedAt:p.updatedAt});
...
export async function POST(request:Request){
 ...
 const rawTheme:Record<string,unknown>=p.themeJson&&typeof p.themeJson==="object"?p.themeJson as Record<string,unknown>:{},hex=(v:unknown,fallback:string)=>/^#[0-9a-f]{6}$/i.test(String(v))?String(v):fallback,clamp=(v:unknown,min:number,max:number,fallback:number)=>{const n=Number(v);return Number.isFinite(n)?Math.max(min,Math.min(max,n)):fallback},fonts=new Set(["Verdana","Georgia","Arial","Courier New"]),templates=new Set(["lounge","classic","midnight","neon"]),visibility=rawTheme.moduleVisibility&&typeof rawTheme.moduleVisibility==="object"?Object.fromEntries(Object.entries(rawTheme.moduleVisibility as Record<string,unknown>).slice(0,30).map(([key,value])=>[cleanHandle(key),value!==false])):{},theme={template:templates.has(String(rawTheme.template))?String(rawTheme.template):"lounge",pageBackground:hex(rawTheme.pageBackground,"#24130b"),panelBackground:hex(rawTheme.panelBackground,"#f2dfbe"),textColor:hex(rawTheme.textColor,"#2b160d"),linkColor:hex(rawTheme.linkColor,"#6e2b0e"),headingColor:hex(rawTheme.headingColor,"#fff0cf"),accentColor:hex(rawTheme.accentColor,"#e4862a"),fontFamily:fonts.has(String(rawTheme.fontFamily))?String(rawTheme.fontFamily):"Verdana",showTop8:rawTheme.showTop8!==false,moduleGap:clamp(rawTheme.moduleGap,0,48,16),moduleOpacity:clamp(rawTheme.moduleOpacity,35,100,96),moduleBackground:hex(rawTheme.moduleBackground,"#f2dfbe"),moduleBorderColor:hex(rawTheme.moduleBorderColor,"#e4862a"),moduleVisibility:visibility};
 ...
 const customHtml=await cacheLayoutAssets(text(p.customHtml,20000),wallet),values={handle,displayName:text(p.displayName,40)||existing?.displayName||handle,bio:text(p.bio,500),hobbies:text(p.hobbies,500),interests:text(p.interests,500),music:text(p.music,500),heroes:text(p.heroes,500),lookingToMeet:text(p.lookingToMeet,500),avatarUrl:savedAvatar,bannerUrl:httpsUrl(p.bannerUrl),mood:text(p.mood,40)||"feeling board",moodText:text(p.moodText,140)||"holding down the lumberyard.",customHtml,mobileCss:text(p.mobileCss,12000),themeJson:JSON.stringify(theme),layoutJson:JSON.stringify(layout).slice(0,4000),featuredVideo:youtubeUrl(p.featuredVideo),moderationStatus:existing?.moderationStatus||(isAdmin||autoApprove?"approved":"pending")};
