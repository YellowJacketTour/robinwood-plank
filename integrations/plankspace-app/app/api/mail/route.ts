...
async function authorize(request:Request,p:Proof,action:string,resource:string,data:unknown){return await hasOwnerSession(request)?OWNER_WALLET:verifyAndConsumeProof(p,action,resource,await hashJson(data))}
export async function POST(request:Request){
 const p=await request.json() as Proof&{action?:string;recipientHandle?:string;subject?:string;body?:string};
 if(p.action==="list"){
  const data={action:"list"},wallet=await authorize(request,p,"mail:list","inbox",data);if(!wallet)return Response.json({error:"Sign to open Board Mail"},{status:403});
 ...
 const recipientHandle=clean(p.recipientHandle||""),subject=text(p.subject,80)||"Board Mail",body=text(p.body,1000),data={recipientHandle,subject,body},wallet=await authorize(request,p,"mail:send",recipientHandle,data);if(!wallet||!body)return Response.json({error:"Sign a message with 1–1000 characters"},{status:403});
