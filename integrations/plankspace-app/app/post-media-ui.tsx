...
export function MediaAttachment({mediaUrl,mediaType,mediaAlt}:{mediaUrl?:string;mediaType?:string;mediaAlt?:string}){
 ...
 if(mediaType==="audio")return <div className="post-media"><audio src={mediaUrl} controls preload="metadata" aria-label={mediaAlt||"Voice recording"}/></div>;
...
export function MediaComposer({value,onChange,idPrefix}:{value:PostMedia;onChange:(value:PostMedia)=>void;idPrefix:string}){
 ...
 const pick=async(event:ChangeEvent<HTMLInputElement>)=>{
  ...
  const isAudio=/^audio\/(webm|ogg|mp4)(;|$)/.test(file.type);
  if(!isVideo&&!isImage&&!isAudio){setError("Use images, video, or WebM, Ogg, M4A voice recordings.");event.target.value="";return}
  if((isVideo&&file.size>20_000_000)||(!isVideo&&file.size>3_000_000)){setError(isVideo?"Videos must be under 20 MB.":"Images and audio must be under 3 MB.");event.target.value="";return}
  setUploading(true);
  try{
