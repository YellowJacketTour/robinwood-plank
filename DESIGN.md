...
### PlankCrash approved art direction — 2026-09-09
The user approved `.dream-loop/concept.png`: an ivory handheld control dock over a full-height cinematic launch scene, superseding the dense cockpit layout for `public/arcade/crash.html` only. Preserve the original Chalkstronaut painted identity on a dimensional wood character. Use cream and gold controls, cool steel infrastructure and amber lamps. One stake selector with ETH and approximate USD, target and play; detailed accounting remains reachable through tools. Geometry must have actual openings and ball clearance, not transparent shells or result overlays that phase through the model. Other app and marketing surfaces retain their existing rules.
...
### Charmville cartridge shell (September 14)
The game is the primary surface: one compact status/menu bar above a persistent renderer. Journal, world map, audience controls and social tools belong inside the game menu, never stacked above play. The Bag uses item artwork, pockets, a bounded list and a description panel. Menu overlays release gameplay input but retain the mounted renderer and authoritative updates.
The existing scoped game palette in world-shell.css remains the vocabulary: game-night #102F3A, game-frame #174955, game-panel #205966, game-edge #75B7B3, game-paper #F2F7E6, game-muted #B7D5D2, game-select #F8D66D. Light paper panels use game-night text. Native item images are nearest-neighbor; text remains legible at browser scale. Themes must preserve these semantic roles, selected/focus states and contrast.
Original Burning Heart asset-only paint roles retain heart-shadow #BE123C and heart-light #F43F5E, with the existing gold/cream flame and wood outline. These are illustration pigments, not new UI tokens. Preserve a connected silhouette and clear alpha border; show 24px reference icons at integer multiples of their source grid rather than stretching them to 32px.

Original orb-fairy illustration pigments are fairy-core #FFFFFF, fairy-wing #DCEAFF and fairy-aura #789BFF. Warm/leaf variations reuse gold-400, cream and charm-sage #A9C784. These colors belong only to the fairy SVG/art, not an alternative UI palette. Its four wings and core share one anchor; reduced-motion removes animation without hiding the guide.
