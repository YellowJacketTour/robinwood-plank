...
test("PostgreSQL companion selection persists once, isolates owners and rejects replacement",{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const file of ["090_plankspace_native.sql","116_charmville_soil.sql","117_charmville_layout.sql","119_charmville_grain_reserve.sql","122_charmville_companions.sql","123_charmville_companion_following.sql"])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${file}`,"utf8"));
