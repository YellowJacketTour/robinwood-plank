...
test("source turn combat persists HP PP and idempotent server outcomes",{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const f of ["090_plankspace_native.sql","116_charmville_soil.sql","118_charmville_home_access.sql","120_charmville_world_presence.sql","121_charmville_exchange.sql","125_charmville_native_actor.sql","126_charmville_native_resources.sql","122_charmville_companions.sql","123_charmville_companion_following.sql","124_charmville_creature_roster.sql","127_charmville_creature_vitals.sql","128_charmville_encounters.sql","129_charmville_turn_combat.sql","132_charmville_battle_events.sql"])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${f}`,"utf8"));
  ...
  const spectator=await worldEncounter(pool,tokens[1]);assert.equal(spectator.participants.length,2);assert.equal(spectator.events.length,1);assert.ok(spectator.events[0].log[0].targetId);assert.equal(spectator.participants.find(p=>p.boundCreature)?.boundCreature?.id,creatureId);assert.equal((await worldEncounter(pool,tokens[0])).events.length,1);
  ...
  await pool.query("UPDATE charmville_world_presence SET expires_at=clock_timestamp()-interval '1 second' WHERE profile_id=$1",[owner]);const hidden=await worldEncounter(pool,tokens[1]);assert.equal(hidden.events.length,0);assert.equal(hidden.participants.length,1);
