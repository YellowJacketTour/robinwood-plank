...
import { classifyHole, isSchedulable } from "../../components/market/TypedHole";
 ...
 * as the same em-dash, which cannot be distinguished from zero, from "not
 ...
 * in GlobalMarketHub.tsx, whose own comment records that these strings were
 * rewritten once to stop promising data the pipeline could not deliver. They
 * were hidden behind a `title` attribute: invisible until hover, and on touch
 * invisible entirely.
 *
...
test("an observed zero is a FACT, not a hole", () => {
  ...
  // show 0. Showing a dash there is a lie of omission -- it implies we do not
...
test("a chain with no source is unsourced, and never scheduled", () => {
  // Holder counts on Solana/Bitcoin: there is no clean single-call endpoint on
...
test("a change with real sales is UNDERIVED, not unfetched", () => {
  // The Beezie case, measured live: 1,468 sales and no 24h change, because the
  // ledger holds no priced native fills in the prior window. That is the pair
  // invariant working correctly. It needs TIME, not another request.
...
test("an unfetched value is the ONLY schedulable kind", () => {
  // This is the bridge to the attention beam: a rendered hole becomes a work
  // order only when fetching could actually fill it.
...
test("the explanation reaches assistive tech, not just a hover", () => {
  ...
  // bare dash announces as nothing at all, which is how 80 % of the table was
...
test("a hole never outshouts a real value", () => {
  ...
  assert.match(SRC, /TONE/, "kinds must be toned");
