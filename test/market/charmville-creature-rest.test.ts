...
test("owned home rest recovers actual battle HP and PP once",{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const f of ["090_plankspace_native.sql","116_charmville_soil.sql","118_charmville_home_access.sql","120_charmville_world_presence.sql","121_charmville_exchange.sql","125_charmville_native_actor.sql","126_charmville_native_resources.sql","122_charmville_companions.sql","123_charmville_companion_following.sql","124_charmville_creature_roster.sql","127_charmville_creature_vitals.sql","128_charmville_encounters.sql","129_charmville_turn_combat.sql","130_charmville_creature_rest.sql","135_charmville_capture.sql"])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${f}`,"utf8"));
  ...
  // A completed encounter can retain its display lease without blocking care.
  await pool.query("UPDATE charmville_encounters SET controller_id=$1,mode='turn',hp=1,captured=false,lease_until=clock_timestamp()+interval '90 seconds'",[owner]);
  assert.equal((await creatureRest(pool,tokens[0])).reason,'Leave the encounter before resting');
  await assert.rejects(creatureRest(pool,tokens[0],{phase:'begin',requestId:randomUUID(),creatureId}),/Leave the encounter/);
  for(const captured of [false,true]){
   await pool.query('UPDATE charmville_encounters SET hp=$1,captured=$2',[captured?1:0,captured]);
   assert.equal((await creatureRest(pool,tokens[0])).eligible,true);
   const recovery=randomUUID();await creatureRest(pool,tokens[0],{phase:'begin',requestId:recovery,creatureId});
   await creatureRest(pool,tokens[0],{phase:'cancel',requestId:recovery});
   const reserved=(await pool.query('SELECT controller_id::text,mode,hp,captured,lease_until>clock_timestamp() AS live FROM charmville_encounters')).rows[0];
   assert.deepEqual(reserved,{controller_id:owner,mode:'turn',hp:captured?1:0,captured,live:true});
  }
  await pool.query("UPDATE charmville_encounters SET controller_id=NULL,mode='world',hp=max_hp,captured=false,lease_until=NULL");
  ...
  await pool.query("INSERT INTO charmville_encounters(id,region_id,geometry_revision,species_id,level,hp_iv,hp,max_hp) SELECT $1,$2,geometry_revision,species_id,level,hp_iv,max_hp,max_hp FROM charmville_encounters LIMIT 1",[randomUUID(),`home:${owner}`]);
