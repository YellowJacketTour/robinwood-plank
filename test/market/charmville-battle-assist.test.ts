...
test("consented helper contributes once to shared HP without taking controller",{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const f of ["090_plankspace_native.sql","116_charmville_soil.sql","118_charmville_home_access.sql","120_charmville_world_presence.sql","121_charmville_exchange.sql","125_charmville_native_actor.sql","126_charmville_native_resources.sql","122_charmville_companions.sql","123_charmville_companion_following.sql","124_charmville_creature_roster.sql","127_charmville_creature_vitals.sql","128_charmville_encounters.sql","129_charmville_turn_combat.sql","132_charmville_battle_events.sql","133_charmville_battle_assist.sql"])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${f}`,"utf8"));
  ...
  const placement=(await pool.query('SELECT x,y,geometry_revision FROM charmville_native_actors WHERE profile_id=$1',[owner])).rows[0];
  await pool.query('UPDATE charmville_native_actors SET x=20 WHERE profile_id=$1',[owner]);
  assert.equal((await worldEncounter(pool,tokens[1])).canAssist,false);
  await assert.rejects(turnBattle(pool,tokens[1]),/assist/);
  await pool.query('UPDATE charmville_native_actors SET x=$2,y=$3,geometry_revision=$4 WHERE profile_id=$1',[owner,placement.x,placement.y,'stale-geometry']);
  await assert.rejects(turnBattle(pool,tokens[1]),/assist/);
  await pool.query('UPDATE charmville_native_actors SET geometry_revision=$2 WHERE profile_id=$1',[owner,placement.geometry_revision]);
  assert.equal((await worldEncounter(pool,tokens[1])).canAssist,true,'An unconsumed live invitation works again when its controller returns');
