...
/** Legacy sale-average aggregation is retained as an internal projection.
 * Public floor change is independent of sales and requires comparable,
 * timestamped floor evidence; the PostgreSQL behavior is exercised in
 * observed-floor-change.test.ts. */
...
test("the public floor change uses observed floor evidence, not legacy sale averages", () => {
  assert.match(ROUTE, /floorChangePct: floorChange\?\.changePct \?\? null/);
  assert.match(ROUTE, /floorChangeEvidence: floorChange \?\? null/);
  assert.ok(!ROUTE.includes("BigInt(c.previousFloorPriceWei)"), "the previous refresh is not a 24-hour baseline");
});
