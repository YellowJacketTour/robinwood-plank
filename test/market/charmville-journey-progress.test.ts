...
test('journey uses authenticated, committed own-home history and survives spending',{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const file of ['090_plankspace_native.sql','116_charmville_soil.sql','121_charmville_exchange.sql','122_charmville_companions.sql','126_charmville_native_resources.sql','141_charmville_native_crop_identity.sql'])
  ...
  const action=async(actor:string,region:string,kind:string,status='committed',yieldValue:unknown=null)=>{
   await pool.query('INSERT INTO charmville_native_resources(region_id,bed_id) VALUES($1,0) ON CONFLICT DO NOTHING',[region]);
   await pool.query("INSERT INTO charmville_native_actions(profile_id,request_id,payload_hash,region_id,bed_id,kind,resource_revision,region_epoch,sequence,contact_at,expires_at,status,result) VALUES($1,$2,'fixture',$3,0,$4,0,0,0,clock_timestamp(),clock_timestamp()+interval '1 minute',$5,$6::jsonb)",[actor,randomUUID(),region,kind,status,JSON.stringify({yield:yieldValue})]);
  };
  ...
  const expected=['home.claimed','partner.chosen','home.soil.tilled','home.oran.planted','home.oran.watered','home.oran.harvested'];
