...
test("reputation reads real accepted receipts, deduplicates concurrent retries and excludes moderated pines", { skip: !process.env.CHARMVILLE_TEST_DATABASE_URL }, async () => {
  ...
  try {
    for (const name of ["090_plankspace_native.sql", "116_charmville_soil.sql", "117_charmville_layout.sql", "119_charmville_grain_reserve.sql"]) await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${name}`, "utf8"));
