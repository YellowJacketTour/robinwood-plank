...
test("encounter lifecycle preserves identity HP status and isolates control across replay lease and admission",{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const f of ["090_plankspace_native.sql","116_charmville_soil.sql","118_charmville_home_access.sql","120_charmville_world_presence.sql","125_charmville_native_actor.sql","128_charmville_encounters.sql","122_charmville_companions.sql","123_charmville_companion_following.sql","124_charmville_creature_roster.sql","127_charmville_creature_vitals.sql","129_charmville_turn_combat.sql","132_charmville_battle_events.sql"])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${f}`,"utf8"));
  ...
  await assert.rejects(worldEncounter(pool,tokens[0]),/No wild encounter in this home/);
  assert.equal((await pool.query("SELECT 1 FROM charmville_encounters WHERE region_id=$1",[`home:${initial.profileId}`])).rowCount,0);
  // An older saved home encounter remains playable; safe arrival must not erase
  // a player's in-progress creature, HP, status, or control rights.
  await pool.query("INSERT INTO charmville_encounters(id,region_id,geometry_revision,species_id,level,hp_iv,hp,max_hp) SELECT $1,$2,geometry_revision,species_id,level,hp_iv,hp,max_hp FROM charmville_encounters WHERE id=$3",[randomUUID(),`home:${initial.profileId}`,initial.encounter.id]);
