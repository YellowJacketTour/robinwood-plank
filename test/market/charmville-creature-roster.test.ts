...
test("PostgreSQL roster preserves starter and supports six actually owned fixtures",{skip:!process.env.CHARMVILLE_TEST_DATABASE_URL},async()=>{
 ...
 try{
  for(const f of ["090_plankspace_native.sql","116_charmville_soil.sql","122_charmville_companions.sql"])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${f}`,"utf8"));
  ...
  const migration=await readFile("deploy/inmotion/postgres/migrations/124_charmville_creature_roster.sql","utf8");await pool.query(migration);await pool.query(migration);
  for(const f of ['123_charmville_companion_following.sql','134_charmville_individual_following.sql'])await pool.query(await readFile(`deploy/inmotion/postgres/migrations/${f}`,'utf8'));
  ...
  await pool.query(await readFile('deploy/inmotion/postgres/migrations/134_charmville_individual_following.sql','utf8'));assert.equal((await creatureRoster(pool,token)).slots[1].following,true,'migration rerun preserves independent preference');
