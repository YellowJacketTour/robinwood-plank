 ...
 */
contract FailureBeacon is IDrandBeacon {
    ...
    bool public brokenSchedule;
    bool public brokenRead;
    function setBroken(bool schedule_,bool read_) external {brokenSchedule=schedule_;brokenRead=read_;}
    ...
    function nextRoundAfter(uint256 timestamp) external view override returns (uint64) {
        require(!brokenSchedule,"schedule offline");
    ...
    function randomnessOrZero(uint64 round) external view override returns (bytes32) {
        require(!brokenRead,"read offline");
