...
//  PlankCrashDrandTestbed: the canonical PlankCrashDrand + a seedless private-alpha
//  capability (immutable seedingEnabled=false makes seed==0 a bytecode invariant)
//  + a fail-closed chain-id allowlist (31337/46630 only). The production
//  PlankCrashDrand carries NONE of this surface. See
//  docs/marketplank/DESIGN-PLANKCRASH-SEED-DISABLE-TESTONLY-2026-08-31.md.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
contract PlankCrashDrandTestbed is ReentrancyGuard, PullPayment {
    // ── TEST-ONLY private-alpha seedless vehicle. NEVER DEPLOY TO MAINNET. ──
    // Adds an immutable seedingEnabled flag (false => _computeSeed returns 0 as
    // a BYTECODE invariant, independent of reserve/floor/cap/overflow) and an
    // allowlist chain-id guard (fail-closed: only 31337/46630). IS_TEST_BUILD
    // is exposed for the UI. The production PlankCrashDrand has NONE of this.
    bool public immutable seedingEnabled;
    bool public constant IS_TEST_BUILD = true;
    error TestBuildWrongChain();
    error ProdBuildMustSeed();
    enum Phase {
        BETTING,
        LIVE,
        CRASHED,
    ...
    constructor(Config memory cfg) {
        ...
        // TEST BUILD: allowlist chain-id (fail-closed) + seedingEnabled wiring.
        if (!(block.chainid == 31337 || block.chainid == 46630)) revert TestBuildWrongChain();
        seedingEnabled = cfg.seedingEnabled;
        // When seeding is disabled the seed fraction is irrelevant (the _computeSeed
        // short-circuit never reads it) — and the bootstrap budget must be zero.
        if (cfg.seedingEnabled) {
            if (cfg.seedDenominator == 0 || cfg.seedNumerator == 0 || cfg.seedNumerator >= cfg.seedDenominator) {
                revert BadVaultConfig();
            }
        } else {
            if (cfg.seedBootstrapBudgetWei != 0) revert BadVaultConfig(); // zero-bootstrap invariant
        }
    ...
    function _computeSeed() private view returns (uint256 seed) {
        if (!seedingEnabled) return 0; // TEST BUILD: seed disabled as a bytecode invariant
