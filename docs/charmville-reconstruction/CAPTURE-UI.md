...
## Fullscreen capture controls

The native Game menu now has a visible Throw ball button, enabled only by the parent’s current authenticated availability. Enter opens that existing menu; its existing touch, arrow-key and controller navigation remains in use, without stealing a combat key. An explicit button activation sends only a capture request. The parent validates iframe source and origin, then calls the same guarded CapturePanel command used by the account button. Pending, unavailable and uncertain retry states disable the shortcut; standalone reference play has no capture authority. Account changes clear availability.

`verify-fullscreen-capture.mjs` passed a real local-account run entirely through this native menu: fullscreen remained active during an unsuccessful throw followed by a successful throw, both committed receipts produced native START/OPEN/RESULT and completion acknowledgments, finite balls were consumed, and the captured creature appeared in owned Party storage. This adds access to the existing capture transaction, not a new input-side authority or a verified full character throwing pose.
