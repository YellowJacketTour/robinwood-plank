...
### September 12 placement recovery

Fixed a reconnect mismatch between the parent movement controller and the still-running native script: new correction numbers now advance beyond the observed native receipt. Unacknowledged corrections retry at a bounded 1.5-second interval while grounded; movement remains blocked until acknowledgment. The native script now consumes correction numbers only after applying placement, so airborne/script-deferred placement is not permanently discarded. The rebuilt native quest compiled and was published locally. Twelve movement regression tests passed; the complete test command passed (Node suite: 1,383 passed, 69 skipped). Narrative sequencing and remaining authored placement issues still require further playtesting; these corrections do not establish that all gameplay glitches are resolved.

### September 12 progressive journey audit

Journey now derives harvest guidance from actual account Oran Berry holdings and links directly to the Satchel when berries are available. Holdings are not represented as proof of completing a harvest quest. The following blockers were confirmed in source: welcome resets on every native run; home and public admissions share dmap 4/screen 63; crossing another native screen is outside saved movement support; coordinate correction cannot warp back across native screens. Required next work is an acknowledged arrival/map transition contract and persistent tutorial state, with returning-account, first-account, home/public/visitor, airborne and off-map tests. Do not claim these blockers solved by the Journey copy change.


Arrival recovery implementation: the account movement hook opts into a first-observation warp request when a newly admitted controller sees an unsupported native map. Native placement now invokes the documented Hero->Warp API to the supported map and leaves its receipt pending until placement succeeds. Pending arrivals retry at a bounded rate; after completion, ordinary unsupported-screen exploration still does not change saved regions. This does not cover selecting the same already-active admission again, because the current hook keys off region changes. Native compilation and movement regressions are checked; end-to-end off-map visual traversal remains unverified.

### September 12 persisted introduction

Added authenticated GET/POST tutorial presentation preference, migration 128 (applied to the local database), account hook, origin/source-checked runtime bridge and native completion marker. Completion is not a gameplay milestone and grants no inventory or XP. Native compilation, production build and full suite passed; two database preference tests also passed with no skips. Browser first-run introduction renders on a fresh local profile. Completion/reload behavior is not yet visually verified: automated key taps did not advance the native dialogue during this pass. Launcher now uses full document navigation so the world's isolation headers are applied before embedding the native player; this addresses the soft-navigation blocked-frame path observed during testing. Distinct world maps and authoritative tutorial gameplay milestones remain pending.

### September 12 integrated audit and interaction checkpoint

Returning-player browser reload also verified that the completed introduction does not replay. Full suite: 1,388 passed, 70 skipped, zero failures (1,458 tests); targeted tutorial bridge and PostgreSQL encounter regressions passed. Targeted UI ESLint, required lint and production build passed. Native quest compiled and was published locally. The development server was restarted after the build.
