...
### 7.11 Dev-fund allocation via PLANK market-buy — final, explicitly a different trust model

On every index-coin mint, a small, governed, capped percentage of the payment routes to market-buying PLANK (the live, existing MarketplankVaultV3 ecosystem's reward-weight-burn token) and holds it in a dev-fund treasury for future use. No conflict with PLANK's existing rule elsewhere ("burned separately to direct reward weight; never a basket asset") — this is a separate purchase-and-hold action alongside minting, not a change to what the index owns or how PLANK itself functions.

**State plainly what this is, because it's the one deliberate exception to everything else in this document:** every other value-routing mechanism here (§7.3's reserve growth and dividend claim, §7.7's buyback-and-lock) is engineered so that no one, including the team, has spending access to routed value — that's the whole point of the non-decreasing-NAV invariant and the lock-not-renounce pattern. A dev fund is, honestly, the opposite: real, spendable value under team control. That's a legitimate and common pattern elsewhere in DeFi, but it should not be built with the same "no one can touch it" language as everything else, because that would be false.

**What makes this acceptable rather than a quiet trust regression:**
- The percentage is small and has a hard governed ceiling (same bounded-range, timelocked pattern as every other risk parameter in this codebase — recommend a low ceiling, e.g. 2%, decided by governance, not defaulted high).
- The treasury receiving the bought PLANK is itself a timelocked multisig, not a bare EOA — spending out of it should be visible and delayed the same way every other privileged action in this system already is, not instant and silent.
- This is disclosed to users as exactly what it is — a dev-fund allocation — never described using the "no contract risk," "no admin path" language used elsewhere in this document, because that language would not be true here.

**Explicitly not yet decided:** the exact percentage, the treasury's multisig threshold/signer set, and whether the PLANK is ever released to circulating supply or held indefinitely — these are real governance decisions, not settled by this section.

### 7.12 Platform socialfi treasury — final, a third distinct allocation

Alongside the index (§7.2/§7.3) and the dev-fund PLANK buy (§7.11), a governed, timelocked percentage of the fee flow already reaching the index sink (from both swap splits and mint/redeem fees, across every vault) is redirected — before it counts toward index reserve/dividend/buyback accounting — into a separate platform treasury, holding real ETH and collection shares, earmarked to fund future PLANK airdrops tied to a socialfi leaderboard system as the platform evolves.

**Same honesty discipline as §7.11, restated because it's easy to blur these together:** this is a spendable, team-directed treasury, not part of the trustless index mechanics. It should never be described with the "no admin path," "no one can touch it" language used for §7.3/§7.4/§7.7 — that language would be false here. It reuses the exact push-then-opportunistic-reconcile plumbing already built and tested in §7.2 (a plain transfer, atomic, cannot brick a vault) — only the destination address and the governed split percentage are new.

**Explicitly not yet decided:** the exact percentage (governed, timelocked, same bounded-range pattern as everything else), the treasury's own custody structure (recommend the same timelocked-multisig standard as §7.11, not a bare EOA), and the actual shape of the socialfi leaderboard / airdrop-eligibility system itself, which is a separate, later product design, not settled by this section — this section only specifies where the funding for it comes from and how it's collected.
