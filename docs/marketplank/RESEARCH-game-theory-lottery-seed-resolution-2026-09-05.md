...
> **Scope correction, 2026-09-09:** The negative-EV argument below applies to a coalition funding the whole round, not to every coalition in a mixed-player round. `test/contracts/PlankCoalitionDilution.test.ts` demonstrates that a small low-target entrant can collect the surviving-player pot and that a mixed-round house bonus can exceed the entrant's own rake, while aggregate funding caps still hold. These are consequences of the current redistribution rules, not evidence that protected principal was withdrawn. Same-target split invariance does not prove arbitrary multi-target coalition invariance. Also, the player layer returns `D` only when a survivor exists: on an all-bust result it returns zero to players. Lottery growth here is an expected-value statement under the stated assumptions, never a pathwise or perpetual guarantee; an actual prize payout reduces the spendable pool. The former live-progression dependency in current draw terms is separately fixed and tested in `LOTTERY-COMMITMENT-SAFETY-2026-09-09.md`. Do not use this historical document as a universal mainnet-safety proof.
