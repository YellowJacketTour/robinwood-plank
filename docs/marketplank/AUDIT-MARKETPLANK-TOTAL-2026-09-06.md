...
- **Batch G shipped (2026-09-06, owner observations):** a third scheduler role -- the standing lane -- claims only priority <= 60 jobs (discovery, stats, fills) so demand jobs can no longer starve discovery ("no new collections per chain while on screen"); metadata retries are counted (migration 101) and capped at 5, after which a token is 'empty' with the reason, so a collection can finalize instead of hanging at 99.8% (Pudgy Penguins); the hub's chain counts are polled live every 15 s with a growth pulse, and the counts endpoint's cache dropped from 2 minutes to 10 s. Also carries migration 100 (volume provenance columns + hourly price table) and lib/market/asset-price-hourly.ts prepared for the one-sink aggregator. Owner's other observations, answered: Pudgy did not advance while watched because its job shared the single express slot with every other opened collection and the standing lanes were starved; MAYC vs Milady = ~2x tokens on the same slot with gateway-bound per-token fetches (batch fetch is the F10 item); Bitcoin multi-venue listings: Magic Eden's Bitcoin API is gone; OKX/BestInSlot wiring is the E4-bitcoin item.
- **Batch B1 part 2 shipped (2026-09-06):** one aggregator, `updateVolumeFromMarketEvents`, computes 24h/7d/30d sales, native-wei (native + wrapped-native) and USD sums from the sales ledger with wash trades, reverted rows and stream duplicates excluded; `updateCollectionMarketStats` is the single choke point and refuses vendor volume writes while the ledger figure is fresh (`volume_source='ledger'`, migration 100); Seaport fills are priced in USD at the sale's hour (`asset-price-hourly`, spot-fallback labelled) and both the stream flush and the Seaport writer trigger the aggregator for the collections they touched. Postgres test proves sales/volume/USD per window, wash and stream-duplicate exclusion, and vendor-write refusal.
- **Batch B1 part 3 shipped (2026-09-06):** the one sink now covers LooksRare, Blur, X2Y2, Foundation, Sudoswap, Rarible and CryptoKitties fills (`ledger-sink.ts`: confirmed sale row priced in USD at the sale hour, stream duplicate promoted, aggregator flushed per writer pass). Wyvern is excluded: its fill table carries no collection contract. Open: Tensor/Bitcoin settlement writers; Solana/Bitcoin activity branches over the ledger.
- **Batch B6/L6#10 shipped (2026-09-06):** Helius sale rows now carry SOL amount, decimals and USD at the sale hour and trigger the aggregator; the activity route merges ledger sales into the Solana view (deduplicated by transaction against the Magic Eden live feed) and reads the ledger for Bitcoin instead of returning an empty list.
- **Batch B1 part 4 shipped (2026-09-06):** Tensor settlements (mint -> collection via the token projection) and Bitcoin on-chain settlements (collection from the transfer row) record confirmed, USD-priced sales into the one sink with their own chain namespace and flush the aggregator. The sink now spans every writer except Wyvern, whose fill table carries no collection contract.
- **Batch D part 3 + A8 shipped (2026-09-06, Fable agent, self-reported):** D3 foreign-offer approvals/creation go through `sendForeignTransaction` (ensureChain + allowlist + simulate) with registry chain params; the allowlist accepts the conduit controller and OpenSea's conduit; D4 item-offer acceptance re-derives and validates the order client-side, criteria/trait offers are accepted via OpenSea's own `fulfillment_data.transaction` after asserting to == Seaport, value == 0 and signed bid == displayed price (offers route `acceptable` restored for that path only); D7 canary caps gate fulfillment-data, offer-fulfillment-data, submit-offer and floor-listings (fails closed: `FOREIGN_TRADE_CANARY_ENABLED=true` is required on the server or foreign buys/offers return 503 FOREIGN_TRADE_DISABLED); D5-partial cancel of foreign-chain orders via `sendForeignTransaction` from MyPositions; A8 an AbortSignal from `withTimeout` is honoured between iterations in evm-metadata, opensea-membership and opensea-stats lanes. 14 new tests. Not live-verified: OpenSea criteria calldata encoding against a real bid (needs a key + a real offer); `calldata_suffix` not appended; sweep-preview canary check dormant until the client sends `walletAddress`.
- **Batch E/F part 2 (2026-09-07, two Fable agents interrupted by the session limit; partial work kept because it compiles and its tests pass):** F1 OpenRarity-exact scoring (explicit None, two-key sort, RANK ties; golden test), F7 single-gateway rotation with per-host token buckets (test), F10 bulk `unnest` writes for the token projection (test), rarity-index-runner restructured around a `finalizeRarityIfReady`; E2 time-budgeted forward scan, E4 `magiceden-catalog` + `magiceden-alias` lanes with `alias_symbol` (migration 102), E4-bitcoin OKX and BestInSlot readers (`bitcoin-okx-listings.ts`, `bitcoin-bestinslot.ts`, key-gated, `credential-missing` state) and a `bestinslot-stats` lane, E5 `floor_observed_at` + two-miss floor nulling (migration 102), E6 `statsCapable` on the manifest and per-chain `laneHealth` in the index response with a hub banner. Remaining for the relaunched agents: F4 ERC-4906 cursor, F5 coverage triple + 202, F8 rarity GET enqueue, F9 json_uri fallback; E6 freshness dot from floor_observed_at, E2/E4 verification passes.
- **Batch E/F part 3 shipped (2026-09-07, two Fable agents, self-reported):** F4 ERC-4906 rescan with a durable rotating cursor over collections, <=2,000-block chunks, cursor advanced only on success, `_toTokenId` clamped to known supply; F5 honest coverage triple (terminal/traits/image) on the archival stats, the collection route and the bar ("Provisional (N% traits)"), rarity route 202 while provisional; F8 the rarity route enqueues a demand job instead of walking OpenSea in the web process; F9 Solana json_uri fallback when DAS attributes are empty (bounded per page). Catalog verification: all three new lanes have live handlers; Bitcoin listings merge OKX/BestInSlot with honest source labels; hub banner and freshness dot read `laneHealth`/`statsCapable`/`floor_observed_at`. Bug found by the verifying agent and fixed before it reached production: the previously committed `writeSnapshot` INSERT used an ambiguous `$2` type that Postgres rejects, which would have broken every first-time snapshot insert. 44 new tests across both agents. Open: BestInSlot field names unverified against a live key; rarity GET no longer triggers Solana/Bitcoin indexing on demand until those lanes accept a subject.
