...
void main(){
  ...
  float film=.5+.5*cos(11.*field+8.*q+t*.11+h*4.);
  vec3 rainbow=.54+.46*cos(6.283*(film+vec3(0.,.33,.67)));
  vec3 oil=mix(vec3(.018,.009,.035),rainbow, .26+.35*scales);
  vec3 lava=mix(oil,vec3(1.,.12,.015),smoothstep(.76,1.55,field)*(.2+b*.22));
  vec3 koi=mix(lava,vec3(.04,.75,1.),smoothstep(.72,.98,scales)*(.12+m*.18));
  float edge=smoothstep(.03,.0,abs(field-1.05));
  vec3 col=koi+rainbow*edge*(.18+h*.32)+rainbow*pow(scales,9.)*(.05+h*.16);
...
export default function DiscoFluidBackdrop() {
  ...
  useEffect(() => {
    ...
    const uniforms={r:u("r"),t:u("t"),e:u("e"),b:u("b"),m:u("m"),h:u("h"),bias:u("bias")};
    ...
    const draw=()=>{
      const root=document.documentElement, active=root.dataset.melt==="1";
      canvas.hidden=!active;
      if(active){
        const style=getComputedStyle(root); const bias=Number.parseFloat(style.getPropertyValue("--melt-bias"))||.35;
        gl.uniform2f(uniforms.r,canvas.width,canvas.height); gl.uniform1f(uniforms.t,reduced?0:(performance.now()-started)/1000);
        gl.uniform1f(uniforms.e,spectrum.energy); gl.uniform1f(uniforms.b,spectrum.bass); gl.uniform1f(uniforms.m,spectrum.mid); gl.uniform1f(uniforms.h,spectrum.treble); gl.uniform1f(uniforms.bias,bias);
        gl.drawArrays(gl.TRIANGLES,0,6);
      }
      frame=requestAnimationFrame(draw);
    };
    frame=requestAnimationFrame(draw);
    return()=>{cancelAnimationFrame(frame);window.removeEventListener("resize",resize);window.removeEventListener("plank:audio-spectrum",onSpectrum);gl.deleteProgram(program);};
  },[]);
