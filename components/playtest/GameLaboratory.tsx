...
type Snapshot = {
  ...
  simulation: { iteration: string; protectedPrincipal: string; emissionBuffer: string; lottery: { pool: string; committedPrize: string; highWaterPrize: string; draws: string; hits: string }; totals: Record<string, string> };
  ...
  return <div data-market-shell className="site-shell min-h-screen px-1 py-2 text-cream md:px-3">
    <div className="mx-auto max-w-[1500px]">
      ...
      {!selected ? <section className="grid gap-4 md:grid-cols-2">
        ...
        {identity.isAdmin && <section className="mt-3"><Panel title="Admin simulation console">
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
            <label className="text-xs">Economic parameter<select value={policyKey} onChange={(e) => { setPolicyKey(e.target.value); setPolicyValue(String(snap.policy[e.target.value] ?? "0")); }} className="mt-1 min-h-11 w-full rounded border border-line bg-panel-strong px-2">
              {["minimumPlayers","minimumStake","protectedPrincipalBps","keeperRewardBps","crashSeed","emissionBufferCap","lotteryFounderFeeBps","lotteryOddsOneIn","lotteryKappaBps","carveMinBps","carveMaxBps","carveHalfSaturation"].map((key) => <option key={key}>{key}</option>)}
            </select></label>
        ...
        <section className="mt-3 grid gap-3 lg:grid-cols-3"><Metrics title="Heartwood Vault" rows={[["Protected principal", credits(snap.simulation.protectedPrincipal)], ["Emission buffer", credits(snap.simulation.emissionBuffer)], ["Iterations", snap.simulation.iteration]]} /><Metrics title="Progressive lottery" rows={[["Prize on the board", credits(snap.simulation.lottery.committedPrize)], ["Banked (net)", credits(snap.simulation.lottery.pool)], ["High water", credits(snap.simulation.lottery.highWaterPrize)], ["Draws / hits", `${snap.simulation.lottery.draws} / ${snap.simulation.lottery.hits}`]]} /><Metrics title="Rake flow" rows={[["Gross rake", credits(snap.simulation.totals.grossRake)], ["Burn", credits(snap.simulation.totals.burned)], ["Community", credits(snap.simulation.totals.communityFunded)]]} /></section>
