...
type Props = { initialIdentity: Identity | null };
...
export function PasskeyGate({ initialIdentity }: Props) {
  ...
  async function enter() {
    ...
    try {
      const result = await json<Identity>(await fetch("/api/playtest/session", {
        method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ displayName, pin }),
      }));
  ...
  return <section className="rounded-xl border border-line bg-panel p-6">
    <h2 className="text-2xl text-gold-300">Enter the private playtest</h2>
    <p className="mt-2 text-sm text-cream-muted">Choose any username. Use the shared four-digit player PIN, or the private six-digit host PIN for simulation controls.</p>
    ...
    <p id="pin-help" className="mt-2 text-xs text-cream-muted">Players use 4 digits. The host uses 6 digits.</p>
    <button className="mt-5 min-h-11 w-full rounded-md bg-gold-500 px-4 text-xs font-black uppercase tracking-wider text-wood-950 disabled:opacity-40" disabled={busy || !displayName.trim() || ![4, 6].includes(pin.length)} onClick={enter}>{busy ? "Entering…" : "Enter playtest"}</button>
