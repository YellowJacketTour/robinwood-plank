...
start "plankcrash-anvil" /min "%TABLE%\bin\anvil.exe" --port 8547 --host 127.0.0.1 --chain-id 31337 --accounts 20 --balance 10000 --order fifo --state "%TABLE%\state\anvil-state.json" --state-interval 300 --transaction-block-keeper 6000 --block-time 0.1 --mixed-mining --silent
timeout /t 6 /nobreak >nul
cd /d "%REPO%"
set PLANK_PRACTICE_RPC_URL=http://127.0.0.1:8547
start "plankcrash-keeper" /min cmd /c node .next\standalone\ops\plankcrash-table\arcade-preview.mjs ^>^> "%TABLE%\logs\preview.log" 2^>^&1
...
start "plankcrash-gateway" /min cmd /c node .next\standalone\ops\plankcrash-table\invite-gateway.mjs ^>^> "%TABLE%\logs\gateway.log" 2^>^&1
