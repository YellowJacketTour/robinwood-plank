...
export type GlobalArchivalSummary = {
  /** Collections with a REAL known_supply -- the only ones "synced" can even
   * be asked of, honestly. Never the denominator against total tracked
   * collections (most of which have score_method='unknown_supply' and
   * cannot be resolved either way -- counting those as "not synced" would
   * be as dishonest as counting them as synced). */
  verifiableCount: number;
  /** Of the verifiable set, how many meet SYNCED_THRESHOLD. */
  syncedCount: number;
  /** syncedCount / verifiableCount, or null when verifiableCount is 0 --
   * never a fabricated 0%. */
  syncedRatio: number | null;
  /** Total rows in collection_archival_stats, verifiable or not -- shown
   * alongside verifiableCount so a caller can be honest about how much of
   * the catalog this ratio actually speaks for. */
  totalTracked: number;
  asOf: string;
};

/** A collection counts as "verifiably synced" at 98%+ of its real known
 * supply -- not 100% -- because live supply figures can undercount by a
 * point or two from burns/renumbering without any real gap in this app's
 * own hydration. Documented threshold, not a fabricated one. */
const SYNCED_THRESHOLD = 0.98;

/**
 * Real, honest, platform-wide "how much of what we track is actually fully
 * synced" resolution -- built for the nav brand mark (2026-08-26: "it needs
 * to know how to properly resolve when collections are verifiably synced").
 * One aggregate query over collection_archival_stats; the DB is the only
 * source of truth here, same as every other function in this file.
 */
async function computeGlobalArchivalSummary(): Promise<GlobalArchivalSummary> {
  const result = await postgresQuery<{ total: string; verifiable: string; synced: string }>(
    `SELECT
       count(*)::text AS total,
       count(*) FILTER (WHERE score_method = 'supply_ratio')::text AS verifiable,
       count(*) FILTER (WHERE score_method = 'supply_ratio' AND archival_score >= $1)::text AS synced
     FROM collection_archival_stats`,
    [SYNCED_THRESHOLD]
  );
  const row = result.rows[0];
  const totalTracked = Number(row?.total ?? 0);
  const verifiableCount = Number(row?.verifiable ?? 0);
  const syncedCount = Number(row?.synced ?? 0);
  return {
    verifiableCount,
    syncedCount,
    syncedRatio: verifiableCount > 0 ? syncedCount / verifiableCount : null,
    totalTracked,
...
/**
 * Cached wrapper -- this is a full-table aggregate over collection_archival_
 * stats (558k+ rows and growing), so it must never run per-request off the
 * nav bar, which renders on every page. Reuses the same durable-KV
 * singleflight-cache.ts primitive every other market read uses (no
 * `provider` -- this reads our own DB, not a rate-limited third party) with
 * a 5-minute soft TTL / 30-minute hard TTL: this number moves slowly by its
 * nature (whole-catalog completeness), so a few minutes of staleness is
 * honest, not a real gap.
 */
export async function getGlobalArchivalSummary(): Promise<GlobalArchivalSummary> {
  const { getOrRefresh } = await import("@/lib/market/multichain/singleflight-cache");
  return getOrRefresh(
    "archival-ledger:global-summary",
    { softTtlMs: 5 * 60_000, hardTtlMs: 30 * 60_000 },
    computeGlobalArchivalSummary
  );
}
