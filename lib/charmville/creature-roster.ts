...
export type RosterCommand={revision:string;slots:(string|null)[]};
...
export async function creatureRoster(pool:Pool,token:string,raw?:RosterCommand){
 const client=await pool.connect();try{
  ...
  if(raw){const input=parseRoster(raw);const same=JSON.stringify(slots)===JSON.stringify(input.slots);
  ...
  const owned=(await client.query(`SELECT id,source_species_id AS "speciesId",nickname,source,acquisition_kind AS "acquisitionKind" FROM charmville_creature_entities WHERE owner_profile_id=$1 ORDER BY created_at,id`,[profileId])).rows;
