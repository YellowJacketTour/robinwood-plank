...
const starterMoves:Record<number,number>={277:1,280:10,283:33};
...
export async function turnBattle(pool:Pool,token:string,raw?:unknown){await creatureVitals(pool,token);const c=await pool.connect();try{
 ...
 if(raw!==undefined){const q=raw as Record<string,unknown>|null;if(!q||q.action!=="attack"||!uuid.test(String(q.requestId))||!uuid.test(String(q.creatureId))||typeof q.revision!=="string"||Object.keys(q).some(k=>!["action","requestId","encounterId","revision","actorEpoch","creatureId","moveId"].includes(k)))throw new YardError("Invalid attack",400);
 const hash=createHash("sha256").update(JSON.stringify([q.encounterId,q.revision,q.actorEpoch,q.creatureId,q.moveId])).digest("hex"),prior=(await c.query("SELECT * FROM charmville_battle_receipts WHERE profile_id=$1 AND request_id=$2",[id,q.requestId])).rows[0];
 if(prior){if(prior.payload_hash!==hash)throw new YardError("Request ID already used",409);result=prior.result;}
 else{if(q.encounterId!==e.id||q.revision!==String(e.revision)||q.actorEpoch!==Number(actor.region_epoch))throw new YardError("Battle changed. Refresh",409);const mon=owned.find(r=>r.id===q.creatureId);if(!mon||w.partner_id&&w.partner_id!==mon.id)throw new YardError("Choose the bound owned starter",403);
 const stats=(await c.query("SELECT * FROM charmville_creature_combat WHERE creature_id=$1 FOR UPDATE",[mon.id])).rows[0];if(q.moveId!==stats.move_id||stats.pp<=0||mon.hp<=0||e.hp<=0||w.pp<=0)throw new YardError("This attack is unavailable",409);
 const log:{actor:string;move:string;damage:number;critical:boolean;miss:boolean}[]=[];
 const strike=(player:boolean)=>{if(mon.hp<=0||e.hp<=0)return;const move=moveStats(player?stats.move_id:33)!;if(player)stats.pp--;else w.pp--;const miss=randomInt(100)>=move.accuracy,critical=!miss&&randomInt(16)===0;const damage=miss?0:normalDamage(player?mon.level:e.level,combatStat(player?mon.species:e.species_id,"baseAttack",player?mon.level:e.level,player?stats.attack_iv:w.attack_iv),combatStat(player?e.species_id:mon.species,"baseDefense",player?e.level:mon.level,player?w.defense_iv:stats.defense_iv),move.power,critical,100-randomInt(16));if(player)e.hp=Math.max(0,e.hp-damage);else mon.hp=Math.max(0,mon.hp-damage);log.push({actor:player?mon.id:e.id,move:move.name,damage,critical,miss});};
 const speed= combatStat(mon.species,"baseSpeed",mon.level,stats.speed_iv)-combatStat(e.species_id,"baseSpeed",e.level,w.speed_iv);const first=speed>0||speed===0&&randomInt(2)===0;strike(first);strike(!first);
