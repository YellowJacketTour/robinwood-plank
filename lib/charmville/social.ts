...
export function parseSocialPin(raw: unknown) {
  ...
  if (!p || p.face !== "oran-berry" || !/^[1-9]\d{0,17}$/.test(String(p.postId)) ||
      !/^[\da-f]{8}(-[\da-f]{4}){3}-[\da-f]{12}$/i.test(String(p.requestId)))
    throw new YardError("Choose an owned Oran and a current post", 400);
  return { postId: String(p.postId), face: "oran-berry" as const, requestId: String(p.requestId) };
}
...
export async function charmSocial(pool: Pool, token: string, raw?: unknown) {
  ...
  try {
    ...
    if (pin) {
      ...
      const debit = await c.query("UPDATE charmville_stacks SET qty=qty-1 WHERE profile_id=$1 AND face_id='oran-berry' AND qty>=1 RETURNING qty::text",[id]);
      if (!debit.rowCount) throw new YardError("Harvest an Oran before pinning one",409);
      ...
      await c.query(`INSERT INTO charmville_receipts(id,profile_id,action,request_id,payload_hash,actor_profile_id,actor_wallet,session_hash,face_id,qty,result)
        VALUES($1,$2,'stamp',$3,$4,$2,$5,$6,'oran-berry',1,$7::jsonb)`,[receiptId,id,pin.requestId,payloadHash,actor.wallet,hash(token),JSON.stringify(result)]);
      await c.query("INSERT INTO charmville_stamps(receipt_id,profile_id,post_id,face_id,qty) VALUES($1,$2,$3,'oran-berry',1)",[receiptId,id,pin.postId]);
    ...
    const posts = await c.query(`SELECT p.id::text,p.body,a.handle AS "authorHandle",a.display_name AS "authorName",p.created_at AS "createdAt",
      ...
      JOIN plankspace_profiles v ON v.id=$1 WHERE ${eligible} ORDER BY p.id DESC LIMIT 30`,[id]);
    const balance = (await c.query("SELECT qty::text FROM charmville_stacks WHERE profile_id=$1 AND face_id='oran-berry'",[id])).rows[0]?.qty ?? "0";
    await c.query("COMMIT");
    return { posts:posts.rows, basket:[{face:"oran-berry",qty:balance}] };
