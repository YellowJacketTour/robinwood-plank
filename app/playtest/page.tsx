...
import { adminConfigured, currentPlaytestIdentity, playtestBootstrapAllowed, playtestEnabled, playtestInvitePreview, playtestPublicRegistrationEnabled } from "@/lib/playtest-auth";
  ...
  return <>
    ...
    <main id="main-content" tabIndex={-1} className="flex-1 px-3 py-6 sm:px-5 sm:py-10">
      <div data-market-shell className="mx-auto w-full max-w-[1100px] space-y-8">
        ...
        <div className="max-w-2xl"><PasskeyGate publicRegistration={playtestPublicRegistrationEnabled()} adminConfigured={configured} initialInvite={initialInvite} initialSetup={initialSetup} initialIdentity={identity ? { displayName: identity.displayName, isAdmin: identity.isAdmin } : null} invitePreview={invitePreview} /></div>
