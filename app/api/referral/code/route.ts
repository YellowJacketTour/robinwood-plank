...
/**
 * Allocate this wallet's invite code. POST because it writes.
 *
 * This used to happen inside GET /api/referral/me, which takes any wallet
 * from a query string — so reading someone's referral info minted a row for
 ...
 * pointer that credits its owner when someone else uses it, so minting one
...
export async function POST(req: Request) {
  try {
    ...
    const limited = rateLimit(req, { key: "referral-code-mint", limit: 10, windowMs: 60_000 });
