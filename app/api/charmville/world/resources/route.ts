...
export async function GET(r:Request){try{const policy=nativeResourceRequestPolicy(r);return Response.json(await nativeResources(postgresPool(),token(r),undefined,policy),{headers});}catch(e){return fail(e);}}
export async function POST(r:Request){try{requireRuntimeRequestOrigin(r,{allowMissing:true});const policy=nativeResourceRequestPolicy(r);const text=await r.text();if(text.length>1024)throw new YardError("Action too large",413);let raw:unknown;try{raw=JSON.parse(text);}catch{throw new YardError("Invalid action",400);}return Response.json(await nativeResources(postgresPool(),token(r),raw,policy),{headers});}catch(e){return fail(e);}}
