...
export async function GET(request:Request) {
  if(!familyAcceptanceEnabled(process.env,await acceptedHeartCapability()))return Response.json({available:false},{headers});
  try {
...
export async function POST(request:Request) {
  try {
    ...
    const token=await readFamilyAcceptance(request,process.env,await acceptedHeartCapability());
