...
async function handle(r:Request,provision:boolean){if(!localPlaytestRequestAllowed(r,process.env))return Response.json({enabled:false,eligible:false,granted:false},{status:provision?404:200,headers});try{return Response.json({...(await testParty(postgresPool(),r.headers.get("authorization")?.replace(/^Bearer\s+/i,"")??"",provision)),enabled:true},{headers});}catch(e){return Response.json({error:e instanceof YardError?e.message:"Sandbox party unavailable"},{status:e instanceof YardError?e.status:503,headers});}}
