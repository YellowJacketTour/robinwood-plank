...
export function useNativeResources(frame:RefObject<HTMLIFrameElement|null>,session:RefObject<{token:string}|null>,accountId:string|undefined,admission:string|undefined,onChanged:()=>void,runtimeOrigin=DEFAULT_RUNTIME_ORIGIN){
 ...
 useEffect(()=>{
  ...
  const client=createNativeResourceClient({read:()=>request('/api/charmville/world/resources'),actor:()=>request('/api/charmville/world/actor'),post:body=>request('/api/charmville/world/resources',body),send:body=>{if(!disposed)frame.current?.contentWindow?.postMessage(body,runtimeOrigin);},changed:onChanged,status:message=>{if(!disposed)setStatus(message);},uuid:()=>crypto.randomUUID()});
  const receive=(e:MessageEvent)=>{if(e.origin!==runtimeOrigin||!frame.current||e.source!==frame.current.contentWindow)return;void client.observe(e.data);};
  window.addEventListener('message',receive);const timer=setInterval(()=>{if(!document.hidden&&frame.current)void client.poll();},1000);
  return()=>{disposed=true;clearInterval(timer);client.dispose();abort.abort();frame.current?.contentWindow?.postMessage({type:'charmville:resource-state',active:false,beds:[],seeds:0,produce:0},runtimeOrigin);window.removeEventListener('message',receive);};
