...
const tabs=[['play','Play'],['companions','Party'],['inventory','Bag'],['exchange','Exchange'],['friends','Co-op'],['social','Social'],['journal','Journal'],['map','Map'],['settings','Options'],['workshop','Workshop']] as const;
  ...
  return <main ref={menuRoot} data-market-shell data-playing={identity?'true':'false'} data-world-tab={tab} className="charm-world min-h-screen bg-wood-950 p-3 text-cream sm:p-5">
    ...
    <div className={`world-stage grid gap-4 ${tab!=='play'?'xl:grid-cols-[minmax(320px,1fr)_minmax(0,1.2fr)]':''}`}>
      <section id="panel-play" inert={menuOpen||tab!=='play'||!presence?.active} aria-hidden={menuOpen||tab!=='play'||!presence?.active} role="tabpanel" aria-labelledby="tab-play" className={`world-adventure min-w-0 self-start rounded-xl border border-line bg-panel p-3 ${tab!=='play'&&tab!=='social'?'hidden xl:block':''}`} aria-label="Native adventure camera">
        ...
        {camera&&runtimeUrl?<iframe ref={frame} onLoad={()=>{frameReady.current=true;updateCaptureAvailability(captureAvailable.current);frame.current?.contentWindow?.postMessage({type:'charmville:account-peers',active:true,peers:[]},runtimeOrigin);frame.current?.contentWindow?.postMessage({type:'charmville:host-ready',menuSuite:true},runtimeOrigin);updateFollower(followerSpecies.current);updateFollowers(partySpecies.current,partyCreatureIds.current);updateFormation(followerFormation.current);updateEncounter(encounterSnapshot.current);sendPanel();}} title="Charmville native reference adventure" src={runtimeUrl} sandbox="allow-scripts allow-same-origin allow-downloads" allow="cross-origin-isolated; fullscreen; gamepad; keyboard-map; microphone" allowFullScreen referrerPolicy="no-referrer" className="h-[72vh] min-h-96 w-full rounded-lg border border-line" />:
      ...
      <aside data-social={tab==='social'} className={tab==='play'?'hidden':'world-menu'} aria-label="Account world controls">
        ...
        <div className="flex gap-2" aria-label="Co-op pages"><button className={button} aria-pressed={coopPage==='travel'} onClick={()=>setCoopPage('travel')}>Travel together</button><button className={button} aria-pressed={coopPage==='invitations'} onClick={()=>setCoopPage('invitations')}>My invitations</button></div>
