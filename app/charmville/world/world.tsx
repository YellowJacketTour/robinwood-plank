...
export default function World({localRuntime}:{localRuntime:boolean}) {
  ...
  return <main data-market-shell className="min-h-screen bg-wood-950 p-3 text-cream sm:p-5">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <section className="min-w-0 self-start rounded-xl border border-line bg-panel p-3 xl:sticky xl:top-4" aria-label="Native adventure camera">
        ...
        {camera?<iframe title="Charmville native reference adventure" src="http://localhost:3021/charmville/tutorial/" sandbox="allow-scripts allow-same-origin allow-downloads" allow="cross-origin-isolated; fullscreen; gamepad; keyboard-map; microphone" allowFullScreen referrerPolicy="no-referrer" className="h-[72vh] min-h-96 w-full rounded-lg border border-line" />:
      ...
      <aside className="space-y-4" aria-label="Account world controls">
        ...
        {address&&<CompanionPanel key={`companion:${address}`} wallet={address} />}
        {address&&<ExchangePanel key={`${address}:${identity.handle}`} wallet={address} handle={identity.handle} onChanged={()=>void load()} />}
      </aside>
    </div>}
