...
export default function FamilyGiftPanel({token,onSatchel}:{token:string;onSatchel:()=>void}) {
 ...
 return <section aria-label="Your family gift" className="mt-4 rounded-xl border-2 border-line-strong bg-forest-900 p-4 text-cream">
  ...
  <div className="mt-4 flex flex-wrap gap-2">{gift.accepted?<button className={`${button} bg-gold-500 text-wood-950`} onClick={onSatchel}>Open Satchel</button>:<><button disabled={busy} className={`${button} bg-gold-500 text-wood-950`} onClick={()=>void accept()}>{busy?'Saving your gift…':'Accept family seeds'}</button><button disabled={busy} className={`${button} bg-forest-800 text-cream`} onClick={()=>setLater(true)}>Later</button></>}</div>
