...
export default function ExchangePanel({wallet,handle,onChanged}:{wallet:string;handle:string;onChanged?:()=>void}) {
 ...
 return <section aria-label="Grained Exchange" className="mt-6 rounded-xl border border-line bg-panel p-5 text-cream">
  ...
  <p className="mt-2 text-sm text-cream-muted">Player offers at fixed Grain prices. Sell offers reserve charms; buy offers reserve Grain. Another player must accept your offer. Only the current seven saved Satchel definitions can trade here.</p>
  <form className="mt-4 flex flex-wrap items-end gap-3" onSubmit={event=>{event.preventDefault();try{const qty=whole(quantity,1000000n),cost=qty*whole(price,1000000000n);prepare({kind:"create",requestId:crypto.randomUUID(),side,face,quantity,price},side==="sell"?`Reserve ${qty} ${face} to sell at ${price} Grain each (${cost} Grain if fully accepted).`:`Reserve ${cost} Grain to buy ${qty} ${face} at ${price} Grain each.`);}catch(error){setMessage((error as Error).message);}}}>
   ...
   <label className="grid gap-1 text-sm">Charm<select className={input} value={face} onChange={e=>setFace(e.target.value)} disabled={busy||uncertain}>{definitions.map(d=><option key={d}>{d}</option>)}</select></label>
  ...
  {!offers.length?<p className="mt-4 text-cream-muted">No open offers yet.</p>:<ul className="mt-4 divide-y divide-line">{offers.map(offer=><li className="flex flex-wrap items-center justify-between gap-3 py-4" key={offer.id}><div><strong className="text-gold-300">{offer.owner===handle?"Your":`@${offer.owner}’s`} {offer.side} offer</strong><p>{offer.remaining} {offer.face} · {offer.price} Grain each</p></div>{offer.owner===handle?<button type="button" className={button} disabled={busy||uncertain} onClick={()=>prepare({kind:"cancel",requestId:crypto.randomUUID(),offerId:offer.id},`Cancel this offer and return its remaining reserved ${offer.side==="sell"?`${offer.remaining} ${offer.face}`:`${BigInt(offer.remaining)*BigInt(offer.price)} Grain`}.`)}>Cancel offer</button>:<form className="flex flex-wrap gap-2" onSubmit={event=>{event.preventDefault();try{const qty=whole(fills[offer.id]??"1",BigInt(offer.remaining)),cost=qty*BigInt(offer.price);prepare({kind:"fill",requestId:crypto.randomUUID(),offerId:offer.id,quantity:qty.toString()},offer.side==="sell"?`Pay ${cost} Grain and receive ${qty} ${offer.face} from @${offer.owner}.`:`Give ${qty} ${offer.face} and receive ${cost} Grain from @${offer.owner}.`);}catch(error){setMessage((error as Error).message);}}}><input aria-label={`Quantity for ${offer.owner} ${offer.face} offer`} className={`${input} w-24`} inputMode="numeric" value={fills[offer.id]??"1"} disabled={busy||uncertain} onChange={e=>setFills({...fills,[offer.id]:e.target.value})}/><button className={button} disabled={busy||uncertain} type="submit">{offer.side==="sell"?"Review purchase":"Review sale"}</button></form>}</li>)}</ul>}
 </section>;
}
