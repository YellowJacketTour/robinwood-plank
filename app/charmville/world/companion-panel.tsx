...
type Command={speciesId:number}|{action:"following";companionId:string;following:boolean;revision:string}|{action:"claim-home";requestId:string}|{action:'roster';revision:string;slots:(string|null)[]};
...
export default function CompanionPanel({wallet,handle,onFollower,onFollowers,onPlay,onHomeReady}:{wallet:string;handle:string;onFollower?:(speciesId:number)=>void;onFollowers?:(speciesIds:number[])=>void;onPlay?:()=>void;onHomeReady?:()=>void}) {
 ...
 const request=useCallback(async(command?:Command)=>{
  ...
  try{
   ...
   if(!proof.sessionToken){onFollower?.(0);onFollowers?.([]);setRoster(null);throw new Error("Sign in to view your party.");}
   ...
   const response=await fetch("/api/charmville/companions",{...options,method:command&&!claiming&&!editingRoster?"POST":"GET",body:command&&!claiming&&!editingRoster?JSON.stringify(command):undefined});
   const data=await response.json();if(controller.signal.aborted||current!==version.current)return;
   if(!response.ok){if(response.status===401||response.status===403){setState(null);setRoster(null);onFollower?.(0);onFollowers?.([]);}throw new Error(data.error??"Companions unavailable. Refresh your party to check its state.");}
