...
type Vitals={creatures:{id:string;level:number;hp:number;maxHp:number;revision:string;experience?:number;levelStartExperience?:number;nextLevelExperience?:number|null}[];oranQuantity:string};
...
export default function CompanionPanel({wallet,handle,onFollower,onFollowers,onFormation,onTestProfile,onHomeReady}:{wallet:string;handle:string;onFollower?:(speciesId:number)=>void;onFollowers?:(speciesIds:number[],creatureIds?:string[])=>void;onFormation?:(formation:'close'|'relaxed')=>void;onTestProfile?:(profile:TestProfile)=>void|Promise<void>;onPlay?:()=>void;onHomeReady?:()=>void}) {
 ...
 return <section aria-label="Your companion" className={`${styles.shell} @container`}>
  ...
  <div className={styles.layout}>
   <div className={styles.belt}>
    <ol aria-label="Party belt" className={styles.roster}>{slots.map((creature:Creature|null,index:number)=>{
     const health=vitals?.creatures.find(entry=>entry.id===creature?.id),name=creature?portraitNames[creature.speciesId]:null;
     return <li key={index} aria-label={`Slot ${index+1}: ${creature?creature.nickname:'Empty'}`}><button type="button" aria-label={`View slot ${index+1}: ${creature?.nickname??'Empty'}`} aria-pressed={selectedSlot===index} tabIndex={selectedSlot===index?0:-1} onClick={()=>setSelectedSlot(index)} onKeyDown={event=>{const grid=event.currentTarget.closest('ol');const columns=grid?getComputedStyle(grid).gridTemplateColumns.split(' ').length:3;const delta=event.key==='ArrowRight'?1:event.key==='ArrowLeft'?-1:event.key==='ArrowDown'?columns:event.key==='ArrowUp'?-columns:0;if(delta||event.key==='Home'||event.key==='End'){event.preventDefault();event.stopPropagation();const next=event.key==='Home'?0:event.key==='End'?5:(index+delta+6)%6;setSelectedSlot(next);const list=event.currentTarget.closest('ol');(list?.querySelectorAll('button')[next] as HTMLButtonElement)?.focus();}}} className={`${styles.slot} ${selectedSlot===index?styles.selected:""}`}>
      ...
      <span className={styles.slotName}>{creature?.nickname??'Open slot'}</span>
   ...
   <div className={styles.detail}>
    {selectedCreature&&roster?<>
     ...
     <button type="button" className={`${selectedCreature.following?button:primary} w-full`} disabled={busy} aria-pressed={selectedCreature.following} onClick={()=>void request({action:'roster-following',creatureId:selectedCreature.id,following:!selectedCreature.following,revision:roster.revision})}>{selectedCreature.following?'Stop following':'Walk with me'}</button>
     <p className="mt-2 text-center text-xs text-cream-muted">{selectedCreature.following?'Following you · Other companions keep their orders':'Add this companion to your walking formation'}</p>
     <div role="group" aria-label="Companion details" className="my-4 grid grid-cols-3 gap-1 rounded-lg border border-line p-1">{(['summary','care','organize'] as const).map(tab=><button key={tab} type="button" aria-pressed={detail===tab} tabIndex={detail===tab?0:-1} onKeyDown={event=>{const choices=['summary','care','organize'] as const;const index=choices.indexOf(tab);const next=event.key==='ArrowRight'?(index+1)%3:event.key==='ArrowLeft'?(index+2)%3:event.key==='Home'?0:event.key==='End'?2:null;if(next===null)return;event.preventDefault();event.stopPropagation();setDetail(choices[next]);(event.currentTarget.parentElement?.querySelectorAll('button')[next] as HTMLButtonElement)?.focus();}} onClick={()=>setDetail(tab)} className={`min-h-11 rounded-md px-2 text-sm font-bold capitalize focus-visible:outline-2 focus-visible:outline-gold-300 ${detail===tab?'bg-gold-500 text-wood-950':'text-cream-muted hover:text-cream'}`}>{tab}</button>)}</div>
     {detail==='summary'&&<div>{selectedVitals&&typeof selectedVitals.experience==='number'&&<section aria-label="Companion experience" className="mb-4 border-b border-line pb-3"><h4 className="text-sm font-bold text-gold-300">Experience</h4><p className="my-2 text-sm">{selectedVitals.experience} total XP{selectedVitals.level===100?' · Maximum level':''}</p>{selectedVitals.level<100&&typeof selectedVitals.levelStartExperience==='number'&&typeof selectedVitals.nextLevelExperience==='number'&&selectedVitals.nextLevelExperience>selectedVitals.levelStartExperience&&<><meter aria-label="Experience to next level" min={0} max={selectedVitals.nextLevelExperience-selectedVitals.levelStartExperience} value={Math.max(0,selectedVitals.experience-selectedVitals.levelStartExperience)} className="h-3 w-full accent-gold-500"/><p className="mt-1 text-xs text-cream-muted">{Math.max(0,selectedVitals.nextLevelExperience-selectedVitals.experience)} XP to level {selectedVitals.level+1}</p></>}</section>}<dl className="grid grid-cols-2 gap-x-3 gap-y-2 text-sm"><dt className="text-cream-muted">Role</dt><dd>{selectedCreature.id===partner?.id?'Starter partner':'Party companion'}</dd><dt className="text-cream-muted">Condition</dt><dd>{selectedVitals?(selectedVitals.hp===0?'Fainted':selectedVitals.hp===selectedVitals.maxHp?'Healthy':'Needs care'):'Loading…'}</dd><dt className="text-cream-muted">Travel</dt><dd>{selectedCreature.following?'Walking with you':'Resting in party'}</dd><dt className="text-cream-muted">Collection</dt><dd>{selectedCreature.acquisitionKind==='local-playtest'?'Test party':'Starter companion'}</dd></dl>{size&&<section aria-label="Species reference" className="mt-4 border-t border-line pt-3"><h4 className="text-sm font-bold text-gold-300">Species reference</h4><dl className="mt-2 grid grid-cols-2 gap-2 text-sm"><dt className="text-cream-muted">Height</dt><dd>{(size.heightDm/10).toFixed(1)} m</dd><dt className="text-cream-muted">Weight</dt><dd>{(size.weightHg/10).toFixed(1)} kg</dd></dl><p className="mt-2 text-xs text-cream-muted">Emerald species reference, not measurements of this individual companion.</p></section>}</div>}
