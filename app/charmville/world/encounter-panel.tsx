...
export type Snapshot={canAssist?:boolean;participants?:Participant[];events?:CombatEvent[];profileId:string;actorEpoch:number;inRange:boolean;legalActions:Action[];encounter:{id:string;speciesId:number;name:string;level:number;hp:number;maxHp:number;statuses:string[];mode:'world'|'turn';controllerId:string|null;revision:string}};
...
export default function EncounterPanel({wallet,active,onSnapshot}:{wallet:string;active:boolean;onSnapshot?:(snapshot:Snapshot|null)=>void}){
 ...
 const request=useCallback(async(command?:Command)=>{
  ...
  try{const proof=await savedWalletProof(wallet);if(controller.signal.aborted||version!==epoch.current)return;if(!proof.sessionToken){setState(null);onSnapshot?.(null);return;}
   ...
   setState(data);onSnapshot?.(data);if(command){pending.current=null;setRetry(false);setMessage(command.action==='release'?'Encounter released.':'Encounter updated.');}
 ...
 return <section aria-label="Nearby encounter" className="mt-3 rounded-xl border border-line-strong bg-wood-900 p-4 text-cream"><div className="flex items-center justify-between gap-3"><h3 className="font-display text-xl text-gold-300">{encounter.name}</h3><span>Level {encounter.level}</span></div><EncounterScene wild={encounter} participants={state.participants??[]} events={state.events??[]} controllerId={encounter.controllerId}/><p className="my-2 text-sm text-cream-muted">{encounter.controllerId&&encounter.controllerId!==state.profileId?'Another player is meeting this creature.':encounter.mode==='turn'?'Inspecting this creature. Choose an available companion move below.':'A wild creature is nearby.'}</p>{encounter.mode==='turn'&&(encounter.controllerId===state.profileId||state.canAssist)&&<BattlePanel key={`${wallet}:${encounter.id}`} wallet={wallet} encounterId={encounter.id} actorEpoch={state.actorEpoch} onChanged={refresh} showPartner={!state.participants?.some(player=>player.profileId===state.profileId&&player.boundCreature)}/>}{encounter.mode==='turn'&&encounter.controllerId===state.profileId&&<AssistInvitation key={`${wallet}:${encounter.id}`} wallet={wallet} revision={encounter.revision} actorEpoch={state.actorEpoch} participants={(state.participants??[]).filter(player=>player.profileId!==state.profileId)} onChanged={refresh}/>}<div className="flex flex-wrap gap-2">{state.legalActions.filter(action=>action in labels).map(action=><button key={action} type="button" className="min-h-11 rounded-lg border border-line px-4 py-2 text-gold-300 focus-visible:outline-2 focus-visible:outline-gold-300 disabled:opacity-50" disabled={busy||retry} onClick={()=>{const command={requestId:crypto.randomUUID(),encounterId:encounter.id,revision:encounter.revision,actorEpoch:state.actorEpoch,action};pending.current=command;void request(command);}}>{labels[action]}</button>)}{retry&&<button type="button" className="min-h-11 rounded-lg border border-line px-4 py-2 text-gold-300" disabled={busy} onClick={()=>{if(pending.current)void request(pending.current);}}>Retry encounter action</button>}</div>{message&&<p role="status" className="mt-2 text-sm">{message}</p>}</section>;
}
