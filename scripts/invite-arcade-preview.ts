...
const sessions=new Map<string,Guest>();let funding=Promise.resolve();let inflight=0;let joins=0;let joinWindow=Date.now();
...
createServer(async(req,res)=>{
  ...
  try{
    ...
    if(guest&&guest.expires<Date.now()){sessions.delete(sid!);guest=undefined;}
    if(req.method==='POST'){
    ...
    if(url.pathname==='/api/invite/join'&&req.method==='POST'){
      ...
      for(const [id,g]of sessions)if(g.expires<Date.now())sessions.delete(id);
      ...
      funding=funded.catch(()=>{});await funded;sessions.set(id,g);
...
}).listen(8766,'127.0.0.1',()=>console.log('Invite gateway ready on loopback :8766. Token saved to configured state directory. Simulated ETH only.'));
