...
const panel=document.createElement('dialog');panel.className='charmdex voice-notes';panel.innerHTML=`<form method="dialog" class="voice-studio-heading"><div><small>CHARMDEX · MESSAGES</small><strong>Voice notebook</strong></div><button aria-label="Close voice note">Back</button></form><div class="voice-studio-body"><section class="voice-studio-art"><img src="/menu-art/retro_mail.png" alt="" width="96" height="96"><h2>A little note from you</h2><p>Record a thought.<br>Listen before you share.</p><p role="status">Microphone off · up to 60 seconds</p></section><section class="voice-studio-work"><h2>1 · Make your note</h2><div class="voice-actions"><button type="button" data-record>Record</button><button type="button" data-stop disabled>Stop</button><button type="button" data-play disabled>Play draft</button></div><audio controls hidden></audio><h2>2 · Add a caption</h2><label>Caption or transcript<textarea aria-label="Voice note caption" maxlength="500" rows="4" placeholder="What would you like to say?"></textarea></label><div class="voice-actions"><button type="button" data-copy disabled>Copy caption</button><button type="button" data-dictate>Dictate caption</button></div><p class="voice-recognition-note">Dictation may use your browser’s speech service.</p><h2>3 · Keep your draft</h2><div class="voice-actions"><button type="button" data-save disabled>Save audio</button><button type="button" data-discard disabled>Discard</button></div></section></div><footer class="voice-studio-footer">Nothing is posted here. Save your audio, then attach it in your PlankSpace composer. Live proximity chat is not connected.</footer>`;
panel.setAttribute('aria-label','Voice notebook');
document.body.append(panel);
// Explicit closing also works inside the account shell's form-disabled sandbox.
const close=panel.querySelector('[aria-label="Close voice note"]');close.type='button';close.onclick=()=>panel.close();
...
let recorder=null,stream=null,chunks=[],url='',blob=null,timer=null,elapsed=null,generation=0,recognition=null,disposed=false,returnFocus=null;
...
open.onclick=()=>{if(panel.open)return;returnFocus=document.activeElement;for(const [key,keyCode] of [['ArrowUp',38],['ArrowDown',40],['ArrowLeft',37],['ArrowRight',39],['z',90],['x',88],['d',68],['c',67],['Enter',13],['q',81],['w',87]])document.dispatchEvent(new KeyboardEvent('keyup',{key,keyCode,which:keyCode,bubbles:true}));panel.showModal();get('record').focus();};
...
get('discard').onclick=()=>{discard();status.textContent='Draft discarded · microphone off';get('record').focus();};
...
panel.addEventListener('close',()=>{stop();recognition?.abort();audio.pause();restoreModalFocus(returnFocus);});
