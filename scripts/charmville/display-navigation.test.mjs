...
function setup(){
 const context={};vm.runInNewContext(source.split('// BEGIN native display navigation')[1].split('// END native display navigation')[0],context);
 const events={},keys={};let zoom=1,modal=null,ready=true;
 const canvas={clientHeight:400,addEventListener:(type,fn,options)=>{events[type]=fn;assert.equal(options.passive,false);}};
 ...
 return {events,keys,root,canvas,event,zoom:()=>zoom,setModal:v=>{modal=v;},setReady:v=>{ready=v;}};
}
...
test('pinch changes camera zoom; unavailable renderer does not consume gestures',()=>{
 ...
 s.events.touchcancel();s.events.touchmove(s.event({touches:touches(200)}));assert.equal(s.zoom(),1.5);
