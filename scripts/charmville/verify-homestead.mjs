...
try{
 ...
 for(let i=0;i<2;i++){
  ...
  async function tap(key){await p.keyboard.down(key);await p.waitForTimeout(150);await p.keyboard.up(key);await p.waitForTimeout(850);}
  await p.screenshot({path:out+'/welcome-'+i+'.png'});await tap('d');await tap('d');
