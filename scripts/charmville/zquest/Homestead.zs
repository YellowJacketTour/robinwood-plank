            ...
            if(ticks%6==0){
                ...
                if(correction->isValid()){
                    ...
                    if(request>lastCorrection){
                        ...
                        if(map==4 && screen==63 && (Game->GetCurDMap()!=map || Game->GetCurScreen()!=screen) && Hero->Z==0 && Hero->FakeZ==0){
...
if(follower==286){int phase=(stepX==0 && stepY==0)?0:followerClock%20;int frame=phase<4?0:(phase<8?1:(phase<12?2:(phase<16?3:(4))));int anchor=row*5+frame;drawCompanionScaled(poochyena,286,layer,frame*32,row*48,32,48,fx+8,fy+72,poochyenaAnchorX[anchor],poochyenaAnchorY[anchor]);}
            ...
            if(Game->GetCurDMap()!=4 || Game->GetCurScreen()!=63){faintClock=-1;defeatPending=false;}
            if(encounterVisible==1 && !(captureClock>=34 && captureClock<64+captureShakes*24) && Game->GetCurDMap()==4 && Game->GetCurScreen()==63){
                Screen->DrawOrigin=DRAW_ORIGIN_SCREEN;
                if(encounterHP>0 || defeatPending)drawCompanionScaled(poochyena,286,encounterY<Hero->Y?2:6,0,0,32,48,encounterX+8,encounterY+72,poochyenaAnchorX[0],poochyenaAnchorY[0]);
