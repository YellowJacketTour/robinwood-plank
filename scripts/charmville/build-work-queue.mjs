...
for(const item of items){
 ...
 if(item.feature==='Shared native resource presentation'){item.maturity='███░░';item.status='Oran two-account scope connected; broader acceptance pending';item.evidence='Two actual accounts in one granted private home render native Oran stages 0→1→2→3→4→1; owner gains1 Oran, visitor balances unchanged. Permission-aware native prompts verified; remote tool animation and wider resources remain missing. See NATIVE-RESOURCE-REPLICATION.md.';}
