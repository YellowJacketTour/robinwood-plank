...
const base='http://localhost:3017',out='work/new-player-party-'+name.toLowerCase();
...
try {
 ...
 await party.getByRole('button',{name:'Set up home',exact:true}).click(); await party.getByRole('button',{name}).click();await party.getByRole('button',{name:'Confirm companion'}).click();await party.getByRole('button',{name:'Walk with me',exact:true}).click();await party.getByRole('button',{name:'Return to party',exact:true}).waitFor();
