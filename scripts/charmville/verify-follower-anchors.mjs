...
const script=await readFile('scripts/charmville/zquest/Homestead.zs','utf8')+await readFile('scripts/charmville/zquest/ExtraFollowerFrames.zh','utf8');
...
for(const [id,w,h,frames] of [['0252',32,32,4],['0255',24,32,4],['0258',32,40,6],['0025',32,40,4],['0133',40,48,7],['0261',32,48,5]]){
 ...
 for(let row=0;row<8;row++)for(let frame=0;frame<frames;frame++){
  ...
  const name={'0025':'pikachu','0133':'eevee','0261':'poochyena'}[id];const expected=name?[array(name+'AnchorX')[row*frames+frame],array(name+'AnchorY')[row*frames+frame]]:id==='0258'?[ax[row*6+frame],ay[row*6+frame]]:[w/2,20];
