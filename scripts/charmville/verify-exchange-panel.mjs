...
try {
 ...
 async function pageFor(user){const context=await browser.newContext();await context.addInitScript(({wallet,token})=>{for(const type of ['plank:wallet-state','plank:wallet-response'])window.addEventListener(type,e=>{const state=type==='plank:wallet-state'?e.detail:e.detail?.result?.state;if(state&&state.address!==wallet)e.stopImmediatePropagation();},true);localStorage.setItem('plankspace-last-verified-wallet',wallet);localStorage.setItem('plankspace-session:'+wallet,token);window.addEventListener('plank:wallet-request',e=>{if(e.detail.method==='getState')window.dispatchEvent(new CustomEvent('plank:wallet-response',{detail:{requestId:e.detail.requestId,result:{state:{address:wallet,status:'connected',isConnected:true,chainId:null}}}}));});},user);const page=await context.newPage();await page.goto(base+'/charmville/world');await page.getByRole('tab',{name:'Exchange',exact:true}).click();await page.getByRole('region',{name:'Grained Exchange'}).waitFor({timeout:60000});return page;}
