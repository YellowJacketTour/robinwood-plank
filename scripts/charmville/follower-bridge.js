...
function flush() {
  ...
  try {
    ...
    FS.writeFile(directory + '/party-followers.txt', [...selected,...Array(6-selected.length).fill(0)].join('|'));
    written = projection;
...
window.addEventListener('message', event => {
  ...
  if (!data) return;
  if(data.type==='charmville:follower' && allowedSpecies.has(data.speciesId)) selected=data.speciesId?[data.speciesId]:[];
